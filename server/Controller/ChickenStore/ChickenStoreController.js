...
const updateStoreAvailability = async (req, res) => {
  try {
    ...
    if (!storeId || isOpen === undefined) {
      return res
        .status(400)
        .json({ success: false, message: "Store ID and status are required" });
    }
...
const getMenuItemsByCategory = async (req, res) => {
  ...
  try {
    ...
    if (!menuItems || menuItems.length === 0) {
      return res
        .status(404)
        .json({ message: "No menu items found for the specified filters" });
    }
...
const getMenuItems = async (req, res) => {
  ...
  try {
    ...
    if (!menuItems || menuItems.length === 0) {
      return res
        .status(404)
        .json({ message: "No menu items found for the specified store" });
    }
